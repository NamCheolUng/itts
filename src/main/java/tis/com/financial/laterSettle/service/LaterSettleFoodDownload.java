...
public class LaterSettleFoodDownload extends AbstractExcelView{
	...
	protected void buildExcelDocument(Map<String, Object> excel,
		...
		setFileNmae("후불정산_중식비.xls", request, response);
		
		List allSalaryList = null;
		
		FoodExpensesVO foodVO = new FoodExpensesVO();
		...
		foodVO.setAffiliationId(splitDivInfo[1]);
		foodVO.setFoodDt(splitDivInfo[2]);
		
        HSSFSheet sheet = workbook.createSheet("중식비 관리");
		...
		for(int i=0; i < splitDtDivInfo.length; i++){
			foodVO.setEmplNo(splitDtDivInfo[i]);
				allSalaryList = laterSettleService.selectFoodExpensesListExcel(foodVO);

				List<Map<String, Object>> listMap = allSalaryList; 
				
				String emplNm = listMap.get(0).get("emplNm").toString();
				
				String foodCnt = listMap.get(0).get("foodCnt").toString();
				int nfoodCnt = Integer.parseInt(foodCnt);
