...
public class FileManageDAO extends EgovComAbstractDAO {
	...
	public void updateFileInfsMobile(List<?> fileList) throws Exception {
		FileVO vo;
		Iterator<?> iter = fileList.iterator();
		while (iter.hasNext()) {
			vo = (FileVO) iter.next();
			fileMngService.existDeleteFileInfs(vo);
			insert("FileManageDAO.insertFileDetail", vo);
		}
	}
	/**
	...
	public void existDeleteFileInfs(FileVO fvo) throws Exception {
		delete("FileManageDAO.existDeleteFileInfs", fvo);
	}
	
	@SuppressWarnings("unchecked")
	public List<FileVO> selectMobileDocumentFileList(UserManageVO userManageVO) throws Exception {
		return (List<FileVO>) list("FileManageDAO.selectMobileDocumentFileList", userManageVO);
	}
	
	public String getMobileToPcMaxFileSN(FileVO fvo) throws Exception {
		return (String) select("FileManageDAO.getMobileToPcMaxFileSN", fvo);
	}
}
