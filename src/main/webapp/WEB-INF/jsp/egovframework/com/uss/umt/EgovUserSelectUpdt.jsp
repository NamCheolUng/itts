...
<script type="text/javascript">
	...
	function fn_userIdChk(){
		...
		if (id =="${userManageVO.emplyrId}"){
	 	...
	 	$.ajax({
			url : '<c:url value="/uss/umt/userIdChk.do"/>',
			data : {"checkId":id},
			dataType : 'json',
			type : 'POST',
	...
	function fn_GotoWrite() {
		...
		if(document.subForm.manageAt.value == "Y"){
			if ($("#endChk").is(":checked")) {
				document.subForm.emplyrEndde.value = document.subForm.emplyrEndde1.value
						.replace(/\//gi, "");
		...
		document.subForm.action = "<c:url value='/uss/umt/EgovUserSelectUpdt.do'/>";
		document.subForm.submit();
	}
	
	function fn_GotoCancel(){
		document.subForm.action = "<c:url value='/uss/umt/EgovUserManage.do'/>";
		document.subForm.submit();
	}
</script>
...
<div id="mainwrap">
	<form name="subForm" method="post" enctype="multipart/form-data">
		...
		<input type="hidden" name="returnUrl" value="<c:url value='${tmpPath}/uss/umt/EgovUserSelectUpdtView.do'/>" />
		...
		<input type="hidden" name="manageAt" value="${session.manageAt}">
		...
		<table class="tablewrite">
			...
			<tbody>
				...
				<tr>
					...
					<td><input type="text" name="emplyrId" class="k-textbox width80p" value="${userManageVO.emplyrId }">
				...
				<tr>
					...
					<td><input type="password" name="password" id="password" class="k-textbox width80p" disabled> 
						<labelclass="pwcheck"><input type="checkbox" id="modifyPw" class="k-checkbox"><label class="k-checkbox-label" for="modifyPw">변경</label>
					</td>
					<th>비밀번호확인<i class="fa fa-asterisk" aria-hidden="true"></i></th>
					<td><input type="password" name="password1" id="password1" class="k-textbox width100p" disabled></td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td><c:set var="Bngde" value="${fn:substring(userManageVO.emplyrBngde,0,4)}/${fn:substring(userManageVO.emplyrBngde,4,6)}/${fn:substring(userManageVO.emplyrBngde,6,8)}" />
						...
						<input name="emplyrBngde1" class="datepicker width200" value="${Bngde }"></td>
				...
				<tr>
					...
					<td><input type="checkbox" name="endChk" id="endChk" class="k-checkbox" <c:if test="${not empty userManageVO.emplyrEndde}">checked="checked"</c:if>>
						...
						<div id="emplyrEnddeDiv" style="display: inline;">
							...
							<input name="emplyrEndde1" class="datepicker width200"
		...
		<div class="btngroup fr">
			<button type="button" class="btn02" onclick="javascript:fn_GotoCancel(); ">취소</button>
