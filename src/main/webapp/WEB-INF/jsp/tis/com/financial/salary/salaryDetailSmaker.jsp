...
function fn_goSalaryDetailEmplyr(emplyrNo, affiliationId, emplyrNm){
	...
	document.frm.emplyrNm.value = emplyrNm;
  	...
  	document.frm.action = "<c:url value='/com/financial/salary/salaryDetailEmplyr.do?affili=S'/>";
	document.frm.submit();
}
...
function downloadExcel()
{
	...
	$("input[name=box]:checked").each(function() {
		...
		document.frm.action = "<c:url value='/com/financial/salary/downloadDetailExcel.do?affiliationId=S'/>";
		document.frm.submit();
}

function downloadMonthExcel()
{
		document.frm.action = "<c:url value='/com/financial/salary/salaryMonthDownload.do?affiliationId=S'/>";
		document.frm.submit();
}

function fn_searchSalary(){
	document.frm.ymonth.value = $("#monthpicker2").val().replace("/","");
	document.frm.affiliationId.value = "S";
				...
				<tr>
					...
					<td>	
						...
						<div class="btngroup fr">
							<button type="button" class="btn04" onclick="location.href='<c:url value='/com/financial/salary/salaryList.do?affiliationId=S'/>'">목록</button>
