...
<script>
		...
		$.ajax({
					...
					success : function(data) {
						if (data == 1) {
							...
							document.frm.action = "<c:url value='/com/financial/laterSettle/addEmplToFoodList.do?affiliationId=S'/>";
							document.frm.submit();
						}
					},
	...
	function fn_searchDate() {
		...
		document.frm.action = "<c:url value='${tmpPath}/com/financial/laterSettle/foodExpense.do?affiliationId=S'/>";
		document.frm.submit();
	}
	...
	function fn_downloadExcel() {
		...
		document.frm.action = "<c:url value='/com/financial/laterSettle/downloadExcel.do?affiliationId=S'/>";
		document.frm.submit();
	}
</script>
<div id="subwrap">
	<form name="frm" method="post">
		...
		<div class="tabmenu">
			<ul>
				<li class="active"><a href="<c:url value='/com/financial/laterSettle/foodExpense.do?affiliationId=S'/>">중식비</a></li>
				<li><a href="<c:url value='/com/financial/laterSettle/parkingExpense.do?affiliationId=S'/>">주차정기권</a></li>
			</ul>
		</div>
		<br><br><br> 
	...
	<form name="frm2" id="frm2" method="post">
		<table class="tablelist" id="foodList">
			...
			<tbody>
				<c:forEach var="result" items="${foodExpenseList}"
					varStatus="status">
					<tr id="trList">
						<td><input type="checkbox" name="box" class="checkSelect"
							...
							<input type="hidden" name="jsonEmplNo" value="${result.emplNo }">
		...
		<div class="btngroup fr">
			...
			<input type="hidden" name="mailAddress"> <input type="hidden" name="mailTitle"> <input type="hidden" name="mailText">
			<input type="hidden" name="sMessage"> 
			<input type="hidden" name="sUserId"> 
			<input type="hidden" name="foodCnt"> 
			<input type="hidden" name="returnUrl" value="<c:url value='${tmpPath}com/financial/laterSettle/foodExpense.do?affiliationId=S'/>">
		</div>
		<div id="divLoader">
