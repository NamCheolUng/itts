	...
	function btn_submit(){
		...
		if(f.act.value=='write') {
			if(confirm('저장 하시겠습니까?')) {
				f.action = "<c:url value='/com/epay/draft/epaySalesWriteInsert.do'/>";
				f.submit();
			}
		} else if(f.act.value=='update') {
			if(confirm('수정 하시겠습니까?')) {
				f.action = "<c:url value='/com/epay/draft/epaySalesWriteUpdate.do'/>";
				f.submit();
			}
		}
	}
...
<form name="frm" method="post">
	<c:if test='${salesVO == null}'><input type="hidden" name="act" value="write"></c:if>
	<c:if test='${salesVO != null}'><input type="hidden" name="act" value="update"></c:if>
	...
	<div id="subwrap">
		...
		<h4>
			<i class="fas fa-dot-circle"></i><c:out value="${salesVO == null ? '매출 등록' : '매출 수정'}"/>
		</h4>
		(*) 필수입력항목
			...
			<tbody>
				<tr>
					...
					<td>
						...
						<input type="hidden" name="adbkId" value="<c:out value='${salesVO.adbkId}'/>">
		...
		<div class="btngroup fr">
			<button type="button" class="btn01" onclick="javascript:btn_submit();"><c:out value="${salesVO == null ? '저장' : '수정'}"/></button>
